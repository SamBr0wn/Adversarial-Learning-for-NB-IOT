...
%
% Note that when NPDSCH does not carry the SIB1-NB, an error is triggered
% if the configured |IRep| and |IMCS| values lead to an empty TBS. This is
% the case when the TBS is not defined for a particular |IRep| and |IMCS|
% pair in 3GPP TS 36.213 table 16.4.1.5.1-1 [ <#19 2> ].
...
for repIdx = 1:numel(ireps)
    ...
    %%
    % Verify the configured higher layer parameters using the DL-SCH code rate.
